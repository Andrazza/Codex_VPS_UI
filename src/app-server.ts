...
export class CodexAppServer {
  ...
  private async start(): Promise<void> {
    if (this.starting) return this.starting; if (this.child) return;
    this.starting = (async () => { const child = spawn("codex", ["app-server", "--stdio"], { stdio: ["pipe", "pipe", "pipe"], env: Object.fromEntries(Object.entries(process.env).filter(([key]) => !/SECRET|TOKEN|VPS_|AUTH_REQUIRED/.test(key))) }); this.child = child; createInterface({ input: child.stdout }).on("line", (line) => this.onLine(line)); child.stderr.on("data", (chunk: Buffer) => { const msg = chunk.toString().replace(/\x1B\[[0-?]*[ -/]*[@-~]/g, "").trim(); if (msg) console.error(`codex app-server: ${msg.slice(0, 500)}`); }); child.once("exit", () => this.reset(new Error("Codex app-server stopped"))); child.once("error", (error) => this.reset(error)); await this.request("initialize", { clientInfo: { name: "codex_web_ui", title: "Codex Desk", version: "0.3.0" } }, 12_000); this.notify("initialized", {}); })();
    try { await this.starting; } catch(error) {(this.child as ChildProcessWithoutNullStreams | null)?.kill();this.reset(error instanceof Error?error:new Error("Initialization failed"));throw error;} finally { this.starting = null; }
  }
  private onLine(line: string): void { let message: RpcMessage; try { message = JSON.parse(line); } catch { return; } if (message.method === "account/rateLimits/updated") this.cache = null; if (!message.method && typeof message.id === "number" && this.pending.has(message.id)) { const pending = this.pending.get(message.id)!; clearTimeout(pending.timer); this.pending.delete(message.id); message.error ? pending.reject(new Error(message.error.message ?? "Codex RPC error")) : pending.resolve(message.result); return; } if (message.method && message.id !== undefined) { this.child?.stdin.write(JSON.stringify({id:message.id,error:{code:-32601,message:"This request is not supported by the web UI; action was not approved."}})+"\n"); } if (message.method) for (const listener of this.listeners) listener(message); }
  ...
  private reset(error: Error): void { this.child = null; this.cache = null; for (const pending of this.pending.values()) { clearTimeout(pending.timer); pending.reject(error); } this.pending.clear(); for(const listener of this.listeners)listener({method:"transport/closed"}); }
}